...
pub async fn get_approve_bundle() -> Vec<RelayerTransaction> {
    vec![
...
pub async fn approve_tokens<S: Signer + Send + Sync>(
    ...
    let transactions = get_approve_bundle().await;
    let data = get_multisend_calldata(transactions);
    let packed_signature =
        get_packed_signature(signer.clone(), 1, U256::from(nonce), data.clone()).await?;
